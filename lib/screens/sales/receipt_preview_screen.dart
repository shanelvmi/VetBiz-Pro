...
class _ReceiptPreviewScreenState extends State<ReceiptPreviewScreen> {
  ...
  Future<void> _shareOrPrintA4Pdf() async {
    ...
    try {
      ...
      await ReceiptPdfService.printOrDownloadSale(
        widget.sale,
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // A4 shows a real in-app PDF preview (what you'll actually get)
          // before any print/share action, matching how the thermal
          // receipt is already previewed on screen before those same
          // buttons do anything - built-in actions on PdfPreview itself
          // are turned off, so the existing Share/Print buttons stay the
          ...
          if (!isWide) {
            // Narrow screens: just the receipt, full width, with the
            // action buttons pinned below - the format sidebar and
            // details panel are desktop-only conveniences, not
            // essential to actually viewing or sharing the receipt.
            return Column(
              children: [
  ...
  // The left-hand format picker - only "Receipt (Thermal)" actually
  // changes the receipt's rendering today (a wider "A4 Format" layout
  // isn't built yet, so it's shown but inert rather than silently
  // pretending to do something it doesn't).
  ...
  // The right-hand "Receipt Details" panel, matching the mockup: an
  // icon-labeled summary above, then Quick Actions (Share/Print) below,
  // reusing the exact same _shareOrSave/_print logic the bottom action
  // bar (narrow-screen fallback) also uses.
...
// Draws a zigzag ("torn paper") edge along the top and bottom of the
// receipt, matching how a real thermal-printer receipt tears off -
// purely decorative, doesn't affect hit-testing or the receipt's
// actual content area.
